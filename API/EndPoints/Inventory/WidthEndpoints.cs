...
{
    public static partial class WidthEndpoints
    {
        public static void MapWidthEndpoints(this IEndpointRouteBuilder app)
        {
            var group = app.MapGroup("/api/width").RequireAuthorization();

            // GET all Width
            group.MapGet("/", async (HttpRequest req, IWidthService service) =>
            {
                var query = RegexParseFilterSort.BindPagedQueryDto(req.Query);
                var paged = await service.GetAllAsync(query);
                return Results.Ok(paged);
            }).RequireAuthorization();
            //Get all Width for tabulator
            group.MapGet("/tabulator", GetPagedWidth).RequireAuthorization();

            // GET Width by ID
            group.MapGet("/{id:int}", async (int id, IWidthService service) =>
            {
                var width = await service.GetByIdAsync(id);
                return width is null ? Results.NotFound() : Results.Ok(width);
            });

            // POST create new width
               group.MapPost("/", async (WidthDto dto, IWidthService service) =>
            {
                try
                {
                    var created = await service.CreateAsync(dto);
                    return Results.Created($"/api/width/{created.Id}", created);
                }
                catch (Exception ex)
                {
            ...
            // PUT update Width
            group.MapPut("/{id:int}", async (int id, WidthDto dto, IWidthService service) =>
            {
                try
                {
            ...
            // DELETE Width
            group.MapDelete("/{id:int}", async (int id, IWidthService service) =>
            {
                return await service.DeleteAsync(id) ? Results.NoContent() : Results.NotFound();
            });

            group.MapPatch("/{id}/status", async (int id, [FromBody] short IsActive, IWidthService service) =>
            {
                var updatedWidth= await service.UpdateStatusAsync(id, IsActive);
                return updatedWidth is null
                    ? Results.Problem("Failed to update status")
                    : Results.Ok(updatedWidth);
            }).RequireAuthorization();
        }

        private static async Task<IResult> GetPagedWidth(HttpRequest req, IWidthService service)
        {
