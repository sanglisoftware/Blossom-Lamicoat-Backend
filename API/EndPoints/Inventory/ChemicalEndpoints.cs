...
{
    public static partial class ChemicalEndpoints
    {
        public static void MapChemicalEndpoints(this IEndpointRouteBuilder app)
        {
            var group = app.MapGroup("/api/chemical").RequireAuthorization();

            // GET all chemicals
            group.MapGet("/", async (HttpRequest req, IChemicalService service) =>
            {
                var query = RegexParseFilterSort.BindPagedQueryDto(req.Query);
                var paged = await service.GetAllAsync(query);
                return Results.Ok(paged);
            }).RequireAuthorization();
            //Get all Chemicals for tabulator
            group.MapGet("/tabulator", GetPagedChemical).RequireAuthorization();

            // GET chemical by ID
            group.MapGet("/{id:int}", async (int id, IChemicalService service) =>
            {
                var chemical = await service.GetByIdAsync(id);
                return chemical is null ? Results.NotFound() : Results.Ok(chemical);
            });

            // POST create new Chemical
            group.MapPost("/", async (ChemicalDto dto, IChemicalService service) =>
            {
                try
                {
                    var created = await service.CreateAsync(dto);
                    return Results.Created($"/api/chemical/{created.Id}", created);
                }
                catch (Exception ex)
                {
            ...
            // PUT update Chemical
            group.MapPut("/{id:int}", async (int id, ChemicalDto dto, IChemicalService service) =>
            {
                try
                {
            ...
            // DELETE Chemical
            group.MapDelete("/{id:int}", async (int id, IChemicalService service) =>
            {
                return await service.DeleteAsync(id) ? Results.NoContent() : Results.NotFound();
            });

            group.MapPatch("/{id}/status", async (int id, [FromBody] short IsActive, IChemicalService service) =>
            {
                var updatedChemical = await service.UpdateStatusAsync(id, IsActive);
                return updatedChemical is null
                    ? Results.Problem("Failed to update status")
                    : Results.Ok(updatedChemical);
            }).RequireAuthorization();
        }

        private static async Task<IResult> GetPagedChemical(HttpRequest req, IChemicalService service)
        {
