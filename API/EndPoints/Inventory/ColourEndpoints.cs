...
{
    public static partial class ColourEndpoints
    {
        public static void MapColourEndpoints(this IEndpointRouteBuilder app)
        {
            var group = app.MapGroup("/api/colour").RequireAuthorization();

            // GET all colours
            group.MapGet("/", async (HttpRequest req, IColourService service) =>
            {
                var query = RegexParseFilterSort.BindPagedQueryDto(req.Query);
                var paged = await service.GetAllAsync(query);
                return Results.Ok(paged);
            }).RequireAuthorization();
            //Get all Colour for tabulator
            group.MapGet("/tabulator", GetPagedColour).RequireAuthorization();

            // GET Colour by ID
            group.MapGet("/{id:int}", async (int id, IColourService service) =>
            {
                var colour = await service.GetByIdAsync(id);
                return colour is null ? Results.NotFound() : Results.Ok(colour);
            });

            // POST create new colour
               group.MapPost("/", async (ColourDto dto, IColourService service) =>
            {
                try
                {
                    var created = await service.CreateAsync(dto);
                    return Results.Created($"/api/colour/{created.Id}", created);
                }
                catch (Exception ex)
                {
            ...
            // PUT update Colour
            group.MapPut("/{id:int}", async (int id, ColourDto dto, IColourService service) =>
            {
                try
                {
            ...
            // DELETE Colour
            group.MapDelete("/{id:int}", async (int id, IColourService service) =>
            {
                return await service.DeleteAsync(id) ? Results.NoContent() : Results.NotFound();
            });

            group.MapPatch("/{id}/status", async (int id, [FromBody] short IsActive, IColourService service) =>
            {
                var updatedColour = await service.UpdateStatusAsync(id, IsActive);
                return updatedColour is null
                    ? Results.Problem("Failed to update status")
                    : Results.Ok(updatedColour);
            }).RequireAuthorization();
        }

        private static async Task<IResult> GetPagedColour(HttpRequest req, IColourService service)
        {
