...
{
    public static partial class EnquiryEndpoint
    {
        public static void MapEnquiryEndpoints(this IEndpointRouteBuilder app)

        {
            var group = app.MapGroup("/api/enquiry").RequireAuthorization();

            // GET all enquiey
            group.MapGet("/", async (HttpRequest req, IEnquiryService service) =>
            {
                var query = RegexParseFilterSort.BindPagedQueryDto(req.Query);
                var paged = await service.GetAllAsync(query);
                return Results.Ok(paged);
            }).RequireAuthorization();
            //Get all enquiey for tabulator
            group.MapGet("/tabulator", GetPagedEnquiry).RequireAuthorization();

            // GET enquiry by ID
            group.MapGet("/{id:int}", async (int id, IEnquiryService service) =>
            {
                var enquiry = await service.GetByIdAsync(id);
                return enquiry is null ? Results.NotFound() : Results.Ok(enquiry);
            });

            // POST create new enquiry
            group.MapPost("/", async (EnquiryResponseDto dto, IEnquiryService service) =>
            {
                try
                {
                    var created = await service.CreateAsync(dto);
                    return Results.Created($"/api/enquiry/{created.Id}", created);
                }
                catch (Exception ex)
                {
        ...
        private static async Task<IResult> GetPagedEnquiry(HttpRequest req, IEnquiryService service)
        {
