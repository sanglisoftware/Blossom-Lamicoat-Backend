...
namespace Api.API.EndPoints.Inventory
{
    public static partial class CustomerEndpoints
    {
        ...
        {
            var group = app.MapGroup("/api/customer").RequireAuthorization();

            // GET all Customer
            group.MapGet("/", async (HttpRequest req, ICustomerService service) =>
            {
                var query = RegexParseFilterSort.BindPagedQueryDto(req.Query);
                var paged = await service.GetAllAsync(query);
                return Results.Ok(paged);
            }).RequireAuthorization();
            //Get all Customer for tabulator
            group.MapGet("/tabulator", GetPagedCustomer).RequireAuthorization();
            ...
            // POST create new Customer
               group.MapPost("/", async (CustomerDto dto, ICustomerService service) =>
            {
                try
                {
                    var created = await service.CreateAsync(dto);
                    return Results.Created($"/api/customer/{created.Id}", created);
                }
                catch (Exception ex)
                {
            ...
            // PUT update Customer
            group.MapPut("/{id:int}", async (int id, CustomerDto dto, ICustomerService service) =>
            {
                try
                {
            ...
            group.MapPatch("/{id}/status", async (int id, [FromBody] short IsActive, ICustomerService service) =>
            {
                var updatedCustomer = await service.UpdateStatusAsync(id, IsActive);
        ...
        private static async Task<IResult> GetPagedCustomer(HttpRequest req, ICustomerService service)
        {
