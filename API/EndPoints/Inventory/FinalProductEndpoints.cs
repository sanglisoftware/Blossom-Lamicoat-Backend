...
{
    public static partial class FinalProductEndpoints
    {
        public static void MapFinalProductEndpoints(this IEndpointRouteBuilder app)
        {
            var group = app.MapGroup("/api/finalproduct").RequireAuthorization();

            // GET all FinalProduct
            group.MapGet("/", async (HttpRequest req, IFinalProductService service) =>
            {
                var query = RegexParseFilterSort.BindPagedQueryDto(req.Query);
                var paged = await service.GetAllAsync(query);
                return Results.Ok(paged);
            }).RequireAuthorization();
            //Get all FinalProduct for tabulator
            group.MapGet("/tabulator", GetPagedFinalProduct).RequireAuthorization();

            // GET Gramage by ID
            group.MapGet("/{id:int}", async (int id, IFinalProductService service) =>
            {
                var finalproduct = await service.GetByIdAsync(id);
                return finalproduct is null ? Results.NotFound() : Results.Ok(finalproduct);
            });

            // POST create new gramage
               group.MapPost("/", async (FinalProductDto dto, IFinalProductService service) =>
            {
                try
                {
                    var created = await service.CreateAsync(dto);
                    return Results.Created($"/api/finalproduct/{created.Id}", created);
                }
                catch (Exception ex)
                {
            ...
            // PUT update FinalProduct
            group.MapPut("/{id:int}", async (int id, FinalProductDto dto, IFinalProductService service) =>
            {
                try
                {
            ...
            // DELETE FinalProduct
            group.MapDelete("/{id:int}", async (int id, IFinalProductService service) =>
            {
                return await service.DeleteAsync(id) ? Results.NoContent() : Results.NotFound();
            });

            group.MapPatch("/{id}/status", async (int id, [FromBody] short IsActive, IFinalProductService service) =>
            {
                var updatedFinalProduct = await service.UpdateStatusAsync(id, IsActive);
                return updatedFinalProduct is null
                    ? Results.Problem("Failed to update status")
                    : Results.Ok(updatedFinalProduct);
            }).RequireAuthorization();
        }

        private static async Task<IResult> GetPagedFinalProduct(HttpRequest req, IFinalProductService service)
        {
